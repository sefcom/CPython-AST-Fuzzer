...
TARGETS = [int, float, str, list, tuple,
           dict, set, frozenset, bytes, bytearray]
targets_specs = {"object": obj_specs} | {type(t()).__name__: [(f, get_spec(
    getattr(t(), f))) for f in t().__dir__() if f not in obj_dict] for t in TARGETS}

obj_cnt = sum(len(v) for _, v in targets_specs.items())
body += f"\nbuiltin_clz_obj = malloc(sizeof(PyObject*) * {
    len(targets_specs.keys())});\n"
body += f"overridable_funcs_raw = malloc(sizeof(overridable_func) * {
    obj_cnt});\n"
i = 0
j = 0
...
for k, v in targets_specs.items():
    ...
    for f, spec in v:
        if f in flatten and flatten.index(f) < i:
            body += f"overridable_funcs_raw[{
                i}] = overridable_funcs_raw[{flatten.index(f)}];\n"
        elif spec == []:
            # self
...
codes = codes.replace("const int builtin_type_cnt;",
                      f"const int builtin_type_cnt = {len(targets_specs.keys())};")
